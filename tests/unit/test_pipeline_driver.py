...
class TestPipelineDriver:
    ...
    def test_generate_all_scripts(self, temp_dir, sample_def_file):
        ...
        assert 'test_run_coyote_dryrun' in driver.job_dependencies['test_run_coyote_fillcf']
    ...
    def test_pipeline_validation_success(self, temp_dir, sample_def_file):
        ...
        mock_jobs = [{'job_name': 'valid_job', 'type': 'single', 'script_path': '/tmp/test.sh'}]
    ...
    def test_pipeline_validation_failure(self, temp_dir, sample_def_file):
        ...
        mock_jobs = [{'job_name': 'invalid_job', 'type': 'single', 'script_path': '/tmp/test.sh'}]
        mock_app = MockApplication('invalid_app', mock_jobs)
        
        # Override validation to raise exception
        mock_app.validate_requirements = MagicMock(side_effect=ValueError("Invalid config"))
    ...
    def test_submission_order(self, temp_dir, sample_def_file):
        ...
        mock_jobs = [
            ...
            {'job_name': 'job2', 'type': 'single', 'script_path': '/tmp/job2.sh', 'depends_on_job': 'job1'},
            {'job_name': 'job3', 'type': 'single', 'script_path': '/tmp/job3.sh', 'depends_on_job': 'job2'}
        ]
    ...
    def test_circular_dependency_detection(self, temp_dir, sample_def_file):
        ...
        mock_jobs = [
            {'job_name': 'job1', 'type': 'single', 'script_path': '/tmp/job1.sh', 'depends_on_job': 'job2'},
            {'job_name': 'job2', 'type': 'single', 'script_path': '/tmp/job2.sh', 'depends_on_job': 'job1'}
        ]
    ...
    @patch('subprocess.run')
    def test_submit_pipeline_with_dependencies(self, mock_run, temp_dir, sample_def_file):
