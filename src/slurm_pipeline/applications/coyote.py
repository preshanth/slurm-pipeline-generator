...
class CoyoteJob(SingleJob, ArrayJob):
    ...
    def __init__(self, config_parser, working_dir: str = ".", coyote_binary: str = "coyote"):
        """
    ...
    def validate_coyote_requirements(self):
        ...
        if 'coyote_nprocs' not in self.slurm_config:
            raise ValueError("Missing required parameter: coyote_nprocs in [slurm] section")
    ...
    def create_worker_module(self) -> str:
        ...
        worker_path = self.file_manager.copy_worker_module(worker_source, "coyote_worker.py")
    ...
    def _find_worker_module(self) -> str:
        ...
        possible_locations = [
            Path(__file__).parent.parent / "workers" / "coyote_worker.py",  # Package location
    ...
    def build_worker_command(self, mode: str) -> List[str]:
        ...
        return self.command_builder.build_python_command(str(self.worker_module_path), **args)
